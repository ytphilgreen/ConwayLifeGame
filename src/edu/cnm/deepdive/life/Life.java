...
public class Life {
  ...
  public void populate(double density) {
    boolean[][] work = new boolean[height][width];
  ...
  public void step(Rule rule) {
    ...
    for (int i = 0; i < height; i++) {
      for (int j = 0; j < width; j++) {
        ...
        if (rule.live(field[i][j], neighbors)) {
          work[i][j] = true;
        }
      }
    }
   ...
   * @return the field
   */
  public synchronized boolean[][] getField() {
    boolean[][] safeCopy = new boolean[height][];
    for (int i = 0; i < height; i++) {
   ...
   */
  protected synchronized void setField(boolean[][] field) {
    this.field = field;
  }
